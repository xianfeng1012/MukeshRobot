...
#     else:                         
    try:
        ...
        await message.reply_text(f"以下是几个短链接：",reply_markup=ikm(url))

    except Exception as e:
        await message.reply_text(f"该链接可能已经是短链接或无效链接。")
...
async def unshort(bot, message):
    ...
    try:
        ...
        await message.reply_text(f"以下是还原后的原始链接：\n`{x}` " ,reply_markup=ikm(url))
...
__help__ = """
缩短给定的链接
 ❍ /short <链接> *：示例 `/short https://t.me/mr_sukkun`。
 *"""
